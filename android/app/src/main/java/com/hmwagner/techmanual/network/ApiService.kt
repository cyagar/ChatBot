...
interface ApiService {
    ...
    // Lets ChatViewModel refresh a single conversation's own record directly
    // -- listConversations() fetches every conversation (the wrong tool for
    // "did this one's machine change"), and setConversationMachine()'s
    // response is only reachable via that one action, not via other paths
    // that can resolve a machine (a mention in the question, a
    // clarification answered another way).
