...
|---|---|---|---|
| Model prose cannot state an unsupported qualitative instruction | dev | Open | Claims and steps must be the cited excerpt's own words in order with negations kept, and no-answer text is fixed server text (`app/providers/base.py`, `tests/unit/test_claim_validation.py`). This is a lexical check, not entailment. Remaining: an adversarial safety set run against the live provider and real corpus, and an owner decision whether testers use `local_extractive` instead. |
...
| Gate | Owner | Status | Evidence / what remains |
|---|---|---|---|
| **Deployment topology decided and matching the code** | owner | Owner | Cloud Run is the recorded target, but the scheduler, post-response reindex, local file storage and rate limiter assume one long-lived process; migrations are now serialized by an advisory lock. Choose a single persistent host, or add durable storage, an external scheduler/job, a durable queue and shared rate limiting. Release blocker if Cloud Run stays. |
