...
| Native Android talks to a new Node.js/TypeScript **mobile API** (OIDC/PKCE, transactional outbox, SSE) | Native Android (`android/`) talks **directly to the existing FastAPI backend's JSON endpoints**, over a plain session cookie | Built as a one-day vertical-slice demo (2026-08-23/24) to give technicians something real to react to before committing weeks to Phase 1's infrastructure (Postgres, a managed queue, a secret manager, OIDC — none of which exist yet, per `android/README.md`). Standing up the Node.js API first, with nothing behind it to migrate the FastAPI endpoints to, would have been scaffolding without a payoff for this milestone. | `packages/api-contract` (per the plan) would sit in front of these same FastAPI routes; the Android app's `ApiService`/`ApiModels.kt` already mirror the backend's JSON field-for-field, so retargeting `BASE_URL` at a Node.js API exposing an equivalent contract is a config change, not a rewrite — *except* auth, which is cookie-based (`PersistentCookieJar` in SharedPreferences) rather than the plan's Keystore-backed OIDC/PKCE token vault, and *does* need a real rewrite before this leaves an internal demo. See `android/README.md`'s "Scope decisions" for the full list of what's deferred. |
| Idempotency-key records + a transactional outbox on PostgreSQL (sec 5.1/9/11.1) | An `Idempotency-Key` header on `POST /conversations/{id}/messages`, stored as `messages.idempotency_key` with a `UNIQUE(conversation_id, idempotency_key)` partial index (migration `0010_idempotency.sql`) | The full design needs a durable job queue and an outbox so a dropped connection can *resume* the original in-flight attempt. That's Postgres/Phase-1 infrastructure that doesn't exist yet. The duplicate-question hazard itself (sec 16/17: a retry after a flaky connection must never create a second user turn) doesn't require the queue — it requires knowing "have I seen this key" and "does it already have a reply," both answerable in SQLite because `ask_question` already commits the user-message insert in its own transaction before the slow provider call runs. A duplicate key with no reply yet gets `409` (can't resume the original attempt, but can't safely start a second one either) instead of the plan's resumable-stream behavior. | The `UNIQUE(conversation_id, idempotency_key)` constraint and the "does a reply already exist" lookup (`_reply_to_user_message` in `routes_chat.py`) both translate directly to Postgres — the gap that needs Phase 3's outbox/durable-attempt work isn't the key mechanism, it's turning that `409` into an actual resumable status the client can poll or stream. |
