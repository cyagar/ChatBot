...
class Settings(BaseSettings):
    ...
    # There is no storage_backend setting: only one storage driver (local
    # filesystem) exists, so local_storage_dir alone drives where files
    # actually live (app/ingestion/pipeline.py, app/api/routes_manuals.py).
    # Add a backend-selector setting back if/when a second storage driver is
    ...
    # Google Drive is the only document source -- there is no
    # local-directory ingestion or direct-upload path.
    ...
    # Guardrail against a pathological single file -- generously above any
    # real manual, just bounds a stray multi-GB file. No file-COUNT limit:
    # capping list_files() at N files would make it return a partial listing
    # once a folder passes N, indistinguishable from files having been
    # removed -- see GoogleDriveSource's docstring for why that's
    # deliberately not done.
    max_drive_file_size_mb: int = 200
    # A background loop calls ingest_all() every N minutes so corpus
    # freshness doesn't depend on an admin remembering to click "Run re-index
    # now" -- that stays available as an override, not the only freshness
    # mechanism. 0 disables the loop (e.g. local dev without a configured
    # Drive folder); the loop also never starts unless google_drive_folder_id
    # is set, so tests -- which always leave that blank -- never start it
    # regardless of this default.
    ingestion_sync_interval_minutes: int = 360  # 6 hours
    # How long the corpus can go without a successful sync before the admin
    # UI flags it as stale. Well above the sync interval so one or two
    # missed/slow ticks don't alarm.
    ...
    tesseract_cmd: str = ""
    # Guardrails against a pathological PDF page consuming unbounded
    # CPU/memory -- the Drive file-size cap only bounds download bytes, not
    ...
    # manuals. Set this before any pilot.
    allowed_registration_domains: str = ""

    # Backs GET /api/config. All four are read, not enforced -- a maintenance
    # banner and a minimum-version nudge are UI concerns for whichever client
    ...
    # copied .env.example into .env without replacing this line would boot in
    ...
    # value, independent of length; the example file's own placeholder is
    # also kept short enough to fail the length check too, in case this list
    # is ever incomplete.
    ...
    def validate_for_startup(self) -> None:
        ...
        # Production booting with the wrong document source is structurally
        # impossible: there is no DOCUMENT_SOURCE setting, and
        # get_document_source() (app/ingestion/sources.py) can only ever
        # construct a GoogleDriveSource. FakeDirectorySource
    ...
    def _validate_service_account_key(self) -> None:
        ...
        file was intended) would pass a bare existence check and only fail
        later at the first ingestion attempt.
